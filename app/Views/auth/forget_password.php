...
<div class="flex flex-col mb-5 sm:mb-6">
	...
	<h1 class="headline-animate-gsap text-2xl sm:text-3xl font-black text-brand-dark tracking-tight leading-tight">
		Lupa Password <span class="inline-block bg-gradient-to-r from-brand-primary to-brand-primary_light bg-clip-text text-transparent">Forum Asisten</span>
	</h1>

	<p class="fade-up-gsap text-xs sm:text-sm mt-1.5 text-slate-500 leading-relaxed font-normal">
		Masukkan alamat email Amikom/Universitas yang terdaftar. Kami akan mengirimkan tautan untuk mengatur ulang password akun Anda.
	</p>
</div>

<form action="<?= base_url('auth/processForgetPassword'); ?>" method="POST" class="w-full space-y-4">
	<?= csrf_field() ?>
